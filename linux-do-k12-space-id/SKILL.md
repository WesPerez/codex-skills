...
2. 从已知高产发现路径开始：
   - K12 聚合主题的 reader：`https://r.jina.ai/http://linux.do/t/topic/2514402`；
...
4. 抓取后将 `Published Time` 解析为带时区的 datetime 再排序，不按原始字符串排序。在时间戳缺失或无法解析时，才把 topic ID 降序作为该部分的兜底，并明确标记。
5. 用 Jina reader URL 抓取每个候选：
   `https://r.jina.ai/http://linux.do/t/topic/<id>`
6. 对每个主题记录：
