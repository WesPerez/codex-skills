...
Windows 可执行 shim 使用实际可直接启动的名称，如 `npm.cmd`。不使用 `shell=True`，不把自由命令字符串交给 shell。
...
Checkpoint 在这些时点创建：切片完成、handoff、上下文压缩、长暂停、真实副作用前、commit/push 前和失败阻塞时。
...
首屏必须回答：SAFE/STOP、更新时间、phase/slice/attempt、未决动作、当前有效证据、唯一下一动作、blocker、observed/verified/upstream HEAD、工作树指纹、checkpoint 和技术门禁。技术门禁不等于用户授权。
...
5. 运行 `git --no-optional-locks status --short --ignored`
6. 允许继续后，只读主方案当前 phase/slice
```

轻量 `resume-check` 返回 `light_manual_review`；标准/加固返回技术上的 `safe_for_code_only` 或 `blocked`。它不授予用户写权限或真实副作用。完整 audit 还验证 schema、hash 链、profiles/evidence、checkpoint、STATUS 投影、固定 metadata paths、Git SHA-1/SHA-256 object id 和读取期间并发变化。
...
只在状态变化时写台账：决策、切片开始/结束、测试结果、blocker、用户 steer、真实动作前后、checkpoint、commit/push 和收尾。不要把每次 `rg` 或无变化轮询写成事件。
