...
2. 使用 `python -B` 运行只读 resume-check、完整连续性 audit，并运行 `git --no-optional-locks status --short`；只有已识别出具体 ignored 产物风险时，才对相应路径运行 `git status --short --ignored -- <path>`。自定义台账路径必须把相同的 `--output-dir/--plan-path` 传给两个脚本。
...
把项目验证命令先固定到 `docs/execution/profiles.json`，每个 profile 必须显式声明 `readOnly` 和 `sideEffectClass`；`external_write`/`destructive` 还必须记录窄范围 `externalAuthorization`。再用 `run-evidence --profile <name> --authorization repo_write|external_authorized`。`read_only` workflow 永不运行 `run-evidence`。通用 CLI 不允许自由文本伪造 passed，也不会解锁 `unknown_after_interruption`；加固项目必须补动作专用 verifier。完整契约和命令见 [continuity-ledger.md](references/continuity-ledger.md)。

## 并行和裁决

把源码搜索、日志、测试矩阵、UI、参考项目和资料研究分给窄范围只读子代理。子代理不可用时，用主代理并行只读工具或顺序完成同一矩阵，不因此阻塞。

主代理必须复核结论。纯只读且无网络、下载、进程或外部副作用的子代理用一句话说明结论和证据即可；发生写入或副作用时，再完整列出修改文件、创建/删除、缓存/日志/构建产物、网络/下载、配置/环境变量/凭据、进程/服务、测试、commit hash 和需清理项。
...
最终前必须等待或中断所有子代理，并在当前 workflow 和授权范围内运行可执行的完整验证；`read_only` 只运行经确认无写副作用的检查，且不得调用 `run-evidence`，其他验证报告未运行项及原因。再运行 `git --no-optional-locks status --short`；仅在存在具体忽略产物风险时对对应路径加 `--ignored`，并核对下载、配置、进程、服务、页签和外部状态。只清理能直接证明由本轮创建的目标。报告 commit hash、push 状态、真实运行边界和已知限制。
