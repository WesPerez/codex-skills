...
`429` 是需要分流的状态，不是终止研究的统一信号，也不能自动归因于代理失效。

1. 记录请求 URL 类别、是否携带登录态、HTTP 状态和 `Retry-After`，并复用已经取得的论坛页面、分类/主题 JSON、reader 和 CDN 结果，避免重复请求。论坛原始内容是主证据；外部搜索摘要不能替代原帖。
2. 如果 `Retry-After` 与服务行为一致且可信，遵从等待时间；如果没有可信值，先保持同一身份、同一出口和原有请求形态等待 30 秒后重试一次。任务确实需要且总等待预算仍有余量时可再等 30 秒；单次研究事件的主动等待总计不超过 60 秒。等待期间合并后续请求，不并发补发。
3. 如果错误明确属于账号/全局配额，等待后仍按该作用域降频；换 IP 不能改变这个结论。
4. 如果等待后作用域仍不明且疑似出口级限制：
   - 匿名、公开、幂等的 `GET/HEAD` 可在同一研究事件中串行尝试至多三个彼此独立且已验证的网络路径，例如当前本地代理、短时直连和 reader/CDN；成功后固定该路径，不继续轮换。
   - 携带用户 session 或其他个性化身份时，保持同一 session、用户代理和正常访问频率，每次事件只允许换一个已验证出口做一次 A/B。不要并发请求，也不要让同一账号短时间跨多个公网 IP。
5. 如果有界对照仍为 `429`，停止出口轮换，但继续通过论坛分类/主题 JSON、reader 原文、公开 CDN、已有本地证据或 `Retry-After` 后续跑来完成研究。搜索缓存只能补充候选，不能证明主题发布时间、附件或可用性。

脚本不应使用 `--retry-all-errors` 无间隔重试 HTTP `429`，也不应自动遍历代理池。允许脚本在同一出口按上述 30–60 秒总等待预算延迟重试；出口选择仍由本工作流按身份风险和本次总请求预算决定，并在最终审计中记录。
