...
使用 `references/research_audit_template.md`，但只启用本次实际涉及的区块；没有收藏、附件、下载或浏览器动作时，不创建对应的空审计。
...
2. 不使用浏览器插件发现候选 URL：
   ...
   - 把搜索摘要只当发现线索，不当最终证据。
3. 通过网络方法读取原始主题内容：
   - 尝试 `https://r.jina.ai/http://linux.do/t/topic/<id>`；
...
用户主动提供临时论坛 session 并明确授权时，只把它用于当前任务中确需登录的受限主题或附件解析：使用内存或 `/run` 下 `0600` 的精确临时 cookie 文件，串行低频请求，不写入技能、脚本、shell 历史或长期配置，任务结束立即删除。公开主题和公开附件仍优先走 reader/CDN；session 失效或出现验证码时停止自动重试，不尝试绕过挑战。

应用级 `429` 不应直接终止研究，也不能一律当成代理故障。论坛原始页面、分类/主题 JSON 和公开附件是主证据；reader 只传递论坛原文，外部搜索缓存只用于发现。先记录 `Retry-After`、账号状态和请求范围。可信 `Retry-After` 必须优先遵从；没有可信值时，先保持同一身份和出口等待 30 秒后重试一次，任务确实需要且总等待预算仍有余量时可再等 30 秒，总计不超过 60 秒。明确账号/全局限流时降频、缓存和合并请求。等待后仍疑似出口级限制时，匿名、公开、幂等的 `GET/HEAD` 才可串行尝试至多三个彼此独立且已验证的网络路径；携带 session 的请求每次事件只换一个已验证出口做一次 A/B，避免账号短时间跨多 IP。所有路径仍为 `429` 时停止继续轮换，但继续通过论坛分类/主题 JSON、公开 CDN、已有原文或 `Retry-After` 后续跑完成研究；不得并发扫出口或无限换 IP。连接失败、超时、TLS/出口阻断也只做有上限的换出口重试。
...
- 仅在实际使用浏览器时运行 Edge 标签页审计，关闭已知由任务创建且不再需要的临时标签页，并报告打开、关闭和保留情况；
- 未使用浏览器时只报告“未使用浏览器”，不枚举或审计用户标签页；
- 报告下载和生成文件；
