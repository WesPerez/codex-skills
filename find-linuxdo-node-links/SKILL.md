...
10. 对 Discourse 公开附件优先使用 `linux-do-research` 的确定性 CDN 解析与元数据校验；等级受限帖子仍使用授权登录态读取，不能用 CDN 推导替代正文权限。遇到 `429` 时先在同一身份和出口按 30–60 秒总预算延迟重试；仍被限流才按身份风险预算继续，匿名公开请求最多串行尝试三个已验证网络路径，登录态请求每次事件只做一个新出口 A/B，不并发或无限轮换。结论仍须回到论坛主帖、分类/主题 JSON 或附件证据。
