...
2. 全局语义复扫：整理后重新扫描所有 URL 的名称、URL、当前路径和目标分类，重点检查 `常用/综合`、`工具/其他`、顶级散落项、考试/招聘/下载/云服务/AI/娱乐/开发资料混放，以及明显落在错误技术子类中的非技术资料。
3. 分层候选：把复扫结果分成 `确定可动`、`需要用户判断`、`保留为快捷入口`、`不应移动/误判`。只有 `确定可动` 可以在已授权整理时继续执行；涉及习惯、重复入口或语义不唯一的项必须报告给用户或保留。
4. 复验：再次运行审计脚本，对比备份确认 URL 无丢失、书签栏根部 URL 顺序不变、checksum 有效，并报告移动数量、示例路径变化和仍保留的可疑项。

确定可动的例子：公考题库或国考真题在 `常用/综合`、`SQL/Mysql` 下，应移到考试资料；云服务器控制台在工具云服务下但用户已有常用云服务器入口时，应移到常用云服务器；文件传输、翻译、驱动下载等工具散在综合里，应移到对应工具分类。

需要用户判断的例子：同一 URL 同时作为资料归档和书签栏/常用快捷入口出现；综合里但可能是个人高频入口的页面；语义跨域的技术文章、项目仓库或文档链接。不要为了“看起来干净”强行挪深。
